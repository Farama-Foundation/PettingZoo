...
if __name__ == "__main__":
    ...
    print("===== AgileRL Self-play Agent Demo - Rainbow DQN =====")
    ...
    INIT_HP = {
        ...
        "PRETRAINED": "models/Rainbow_DQN/Rainbow_DQN_random_opp_trained_agent.pt",
    }
    ...
    if INIT_HP["PRETRAINED"] is not None:
        for agent in pop:
            # Load pretrained checkpoint
            agent.loadCheckpoint(INIT_HP["PRETRAINED"])
    ...
    for idx_epi in pbar:
        ...
        if (idx_epi + 1) % evo_epochs == 0:
            ...
            pbar.set_postfix_str(
                f"    Train Mean Turns: {mean_turns}     Eval Mean Fitness: {np.mean(fitnesses)}     Eval Best Fitness: {np.max(fitnesses)}     Total Steps: {total_steps}"
            )
            pbar.update(0)
    ...
    filename = "Rainbow_DQN_self_play_trained_agent.pt"
