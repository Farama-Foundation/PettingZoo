"""Search, evaluate, and render a vector policy for KAZ.

The controller is intentionally small and inspectable. Archers aim using a
basic projectile-intercept calculation, agents prioritize zombies nearest the
bottom of the board, and a compact grid search selects three archer settings.
"""
...
class PolicyParams:
    """Controller parameters; search_params varies only the three archer fields."""
...
def target_for_agent(
    ...
    *,
    ...
    if role_rank is None:
        role_rank = int(agent.rsplit("_", 1)[1])
    if agent.startswith("archer") and params.split_archer_targets:
        return zombies[min(role_rank, len(zombies) - 1)]
...
def policy_action(
    ...
    *,
    ...
    current = observation[0]
    target = target_for_agent(observation, agent, params, role_rank=role_rank)
    if target is None:
        return 5
    ...
    try:
        ...
        for agent in env.agent_iter():
            ...
            if termination or truncation:
                ...
                role = agent.rsplit("_", 1)[0]
                active_role_agents = sorted(
                    other
                    for other in env.agents
                    if other.startswith(f"{role}_")
                    and not env.terminations.get(other, False)
                    and not env.truncations.get(other, False)
                )
                role_rank = (
                    active_role_agents.index(agent)
                    if agent in active_role_agents
                    else 0
                )
        ...
        return EpisodeResult(
            reward=total_reward,
            frames=frames,
            cycles=int(env.unwrapped.frames),
        )
    finally:
        env.close()
...
def evaluate_episodes(
    ...
    *,
    ...
    """Run a policy on an explicit seed block."""
    return [
        run_episode(
            seed,
            params,
...
def evaluate(
    ...
    *,
    ...
    return np.array(
        [
            result.reward
            for result in evaluate_episodes(
                seeds,
                params,
                max_cycles=max_cycles,
                max_zombies=max_zombies,
...
def main() -> None:
    ...
    if (
        args.gif_width <= 0
        or args.gif_colors <= 0
        or args.gif_duration_ms <= 0
        or args.frame_stride <= 0
    ):
        raise ValueError("GIF dimensions, colors, timing, and stride must be positive")
    ...
    if args.search and set(range(args.search_episodes)).intersection(eval_seeds):
        raise ValueError("Search and evaluation seed blocks must be disjoint")

    policy_results = evaluate_episodes(
        eval_seeds,
        params,
        max_cycles=args.max_cycles,
        max_zombies=args.max_zombies,
    )
    random_results = evaluate_episodes(
        eval_seeds,
        params,
        max_cycles=args.max_cycles,
        max_zombies=args.max_zombies,
    ...
    policy_scores = np.array(
        [result.reward for result in policy_results], dtype=np.float64
    )
    random_scores = np.array(
        [result.reward for result in random_results], dtype=np.float64
    )
    policy_cycles = np.array(
        [result.cycles for result in policy_results], dtype=np.int64
    )
    random_cycles = np.array(
        [result.cycles for result in random_results], dtype=np.int64
    )
    ...
    print(f"Policy mean cycles: {np.mean(policy_cycles):.2f}")
    print(f"Random mean cycles: {np.mean(random_cycles):.2f}")
    print(
