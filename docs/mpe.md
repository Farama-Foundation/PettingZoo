...
If an agent cannot see or observe the communication of a second agent, then the values in the observation space that represent the hidden agent are zeroed out in the first's observation space. Observation space sizes are kept static in all environments.
