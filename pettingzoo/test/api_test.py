...
def test_observation_action_spaces(env, agent_0):
    ...
    assert (
        len(env.observation_spaces) == len(env.action_spaces) == len(env.agents)
    ), "observation_spaces, action_spaces, and agents must have the same length"

    for agent in env.agents:
        assert isinstance(
            env.observation_spaces[agent], gym.spaces.Space
        ), "Observation space for each agent must extend gym.spaces.Space"
        assert isinstance(
            env.action_spaces[agent], gym.spaces.Space
        ), "Agent space for each agent must extend gym.spaces.Space"
        if not (
            isinstance(env.observation_spaces[agent], gym.spaces.Box)
            or isinstance(env.observation_spaces[agent], gym.spaces.Discrete)
        ):
            warnings.warn(
                "Observation space for each agent probably should be gym.spaces.box or gym.spaces.discrete"
            )
        if not (
            isinstance(env.action_spaces[agent], gym.spaces.Box)
            or isinstance(env.action_spaces[agent], gym.spaces.Discrete)
        ):
            warnings.warn(
        ...
        if not isinstance(
            env.observation_spaces[agent], env.observation_spaces[agent_0].__class__
        ):
            warnings.warn(
                "The class of observation spaces is different between two agents"
            )
        if not isinstance(
            env.action_spaces[agent], env.action_spaces[agent_0].__class__
        ):
        ...
        if isinstance(env.action_spaces[agent], gym.spaces.Box):
            ...
            if np.any(
                np.equal(env.action_spaces[agent].low, env.action_spaces[agent].high)
            ):
                warnings.warn(
                    "Agent's maximum and minimum action space values are equal"
                )
            if np.any(
                np.greater(env.action_spaces[agent].low, env.action_spaces[agent].high)
            ):
                assert (
                    False
        ...
        if isinstance(env.observation_spaces[agent], gym.spaces.Box):
            ...
            if np.any(
                np.equal(
                    env.observation_spaces[agent].low,
                    env.observation_spaces[agent].high,
                )
            ):
                warnings.warn(
            ...
            if (
                env.observation_spaces[agent].low.shape
                != env.observation_spaces[agent].shape
            ):
                assert (
                    False
                ), "Agent's observation_space.low and observation_space have different shapes"
            if (
                env.observation_spaces[agent].high.shape
                != env.observation_spaces[agent].shape
            ):
                assert (
                    False
...
def play_test(env, observation_0, num_cycles):
    ...
    * tests that the observation is in bounds.
    ...
    for agent in env.agent_iter(env.num_agents * num_cycles):
        ...
        assert not (
            done and agent in has_finished
        ), "agent cannot be done twice in an environment"
        ...
        ), "possible agents should always include all agents"
        if done:
            live_agents.remove(agent)
            has_finished.add(agent)
        assert (
            env.agents == live_agents
        ...
        assert env.observation_spaces[agent].contains(
            prev_observe
