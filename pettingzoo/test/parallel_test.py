...
def parallel_api_test(par_env, num_cycles=10):
    ...
    for n_resets in range(MAX_RESETS):
        ...
        for i in range(num_cycles):
            ...
            assert set(obs.keys()) == (
                set(par_env.agents)
            ), "agents should not be given an observation if they were done last turn"
            assert set(rew.keys()) == (
                set(par_env.agents)
            ...
            assert set(info.keys()) == (
                set(par_env.agents)
            ), "agents should not be given a info if they were done last turn"
            assert set(par_env.agents).issubset(
                set(par_env.possible_agents)
            ), "possible agents should include all agents always"
