...
class env(AECEnv):
    ...
    def __init__(self, **kwargs):
        ...
        self.observation_spaces = self._convert_to_dict([spaces.Box(low=0.0, high=1.0, shape=(6, 34, 4), dtype=np.bool) for _ in range(self._num_agents)])
    ...
    def step(self, action, observe=True):
        if self.dones[self.agent_selection]:
            ...
            if action not in self.infos[self.agent_selection]['legal_moves']:
                ...
                self.infos = self._convert_to_dict([{'legal_moves': [37]} for agent in range(self._num_agents)])
