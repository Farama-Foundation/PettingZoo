...
class env(AECEnv):
    ...
    def __init__(self, **kwargs):
        ...
        self.observation_spaces = self._convert_to_dict([spaces.Box(low=0.0, high=1.0, shape=(7, 4, 15), dtype=np.bool) for _ in range(self._num_agents)])
    ...
    def step(self, action, observe=True):
        if self.dones[self.agent_selection]:
            ...
            if action not in self.infos[self.agent_selection]['legal_moves']:
                ...
                self.infos = self._convert_to_dict([{'legal_moves': [60]} for agent in range(self._num_agents)])
            ...
            if self.env.is_over():
                ...
                if self._int_to_name(next_player_id) == self.prev_player:
                    skip_agent = 0 if self.agent_order.index(self.prev_player) == self._num_agents - 1 else 1
