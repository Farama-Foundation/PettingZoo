...
class env(AECEnv):
    ...
    def __init__(self, **kwargs):
        ...
        self.observation_spaces = self._convert_to_dict([spaces.Box(low=np.zeros(54,), high=np.append(np.ones(52,), [100, 100]), dtype=np.float32) for _ in range(self._num_agents)])
    ...
    def step(self, action, observe=True):
        if self.dones[self.agent_selection]:
            ...
            if action not in self.infos[self.agent_selection]['legal_moves']:
                ...
                self.infos = self._convert_to_dict([{'legal_moves': [2]} for agent in range(self._num_agents)])
