...
class env(AECEnv):
    ...
    def __init__(self, **kwargs):
        ...
        self.observation_spaces = self._convert_to_dict([spaces.Box(low=np.zeros(54,), high=np.append(np.ones(52,), [np.Inf, np.Inf]), dtype=np.float32) for _ in range(self.num_agents)])
