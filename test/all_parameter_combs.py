...
parameterized_envs = [
    (boxing_v2, dict(obs_type="grayscale_image")),
    (boxing_v2, dict(obs_type="ram")),
    (boxing_v2, dict(full_action_space=False)),

    (combat_plane_v2, dict(game_version="jet")),
    (combat_plane_v2, dict(guided_missile=True)),

    (combat_tank_v2, dict(has_maze=True)),
    (combat_tank_v2, dict(is_invisible=True)),
    (combat_tank_v2, dict(billiard_hit=True)),

    (maze_craze_v3, dict(game_version="race")),
    (maze_craze_v3, dict(game_version="capture")),
    (maze_craze_v3, dict(visibilty_level=1)),
    (maze_craze_v3, dict(visibilty_level=3)),

    (space_invaders_v2, dict(alternating_control=True, moving_shields=True, zigzaging_bombs=True, fast_bomb=True, invisible_invaders=True)),
    ...
    (knights_archers_zombies_v10, dict(spawn_rate=50)),
    (knights_archers_zombies_v10, dict(num_knights=4, num_archers=5)),
    (knights_archers_zombies_v10, dict(killable_knights=True, killable_archers=True)),
    (knights_archers_zombies_v10, dict(killable_knights=False, killable_archers=False)),
    (knights_archers_zombies_v10, dict(line_death=False)),
    (knights_archers_zombies_v10, dict(vector_state=False)),
    (knights_archers_zombies_v10, dict(vector_state=False, pad_observation=False)),
    (knights_archers_zombies_v10, dict(max_cycles=100)),
    (knights_archers_zombies_v10, dict(use_typemasks=False)),
    (knights_archers_zombies_v10, dict(max_zombies=2, max_arrows=60)),
    ...
    (tiger_deer_v4, dict(minimap_mode=True)),
    (battle_v4, dict(minimap_mode=False)),
    (battlefield_v5, dict(minimap_mode=False, extra_features=False)),
    (battlefield_v5, dict(minimap_mode=False, extra_features=True)),
    (battlefield_v5, dict(minimap_mode=True, extra_features=False)),
    (battlefield_v5, dict(minimap_mode=True, extra_features=True)),
    (adversarial_pursuit_v4, dict(map_size=15)),
    (battle_v4, dict(map_size=15)),
    (battlefield_v5, dict(map_size=46)),
    (combined_arms_v6, dict(map_size=16)),
    (tiger_deer_v4, dict(map_size=15)),
