...
from paraSharing2 import CustomModel1

env_name = "pistonBall"
# path should end with checkpoint-<> data file
checkpoint_path = "./ray_results/DQN/DQN_pistonBall_c94ea814_2020-01-26_20-31-48wwb8_dt5/checkpoint_10/checkpoint-10"
...
DQNAgent = dqn.DQNTrainer(env=env_name, config=config)
...
while not done:
    ...
    for agent_id in env.agent_ids:
        ...
        action, _, _ = DQNAgent.get_policy("policy_0").compute_single_action(observations[agent_id], prev_reward=rewards[agent_id]) # prev_action=action_dict[agent_id]
        # print(action)
